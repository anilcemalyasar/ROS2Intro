...
class BatteryNode(Node):
    ...
    def __init__(self):
        ...
        self.timer_battery = self.create_timer(0.1, self.check_battery_state)
        
    def get_current_time_seconds(self):
        secs, nanosecs = self.get_clock().now().seconds_nanoseconds()
        return secs + nanosecs / 1000000000
    ...
    def callback_call_set_led(self, future, led_number, state):
        try:
            ...
            state_desc = "On" if state == 1 else "Off"
            battery_desc = "Charging the battery" if state == 1 else "Battery is charged"
            self.get_logger().info("Led " + str(led_number) + 
                                    "is " + state_desc + ". " + battery_desc 
                                    )
